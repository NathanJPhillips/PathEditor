...
internal class DrawablePath
{
    ...
    {
        this.points = new(points ?? throw new ArgumentNullException(nameof(points)));
        if (this.points.Count == 0)
            throw new ArgumentException("Path must contain at least one point", nameof(points));
        Points.Adding += Points_Adding;
    ...
    private void Points_Adding(object sender, CancelEventArgs<Point> args)
    {
        if (args.Value == points[^1])
            args.Cancel = true;
    }

    private void Points_Added(object sender, Point point) =>
